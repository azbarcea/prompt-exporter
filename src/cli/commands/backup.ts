...
import type { ChatGPTClient } from '../../sources/chatgpt/api/client.js';
import {
  BackupService,
  type BackupResult
...
function runBackupWithProgress(
  ...
  options: {
    ...
    getThrottleSummary?: () => string | undefined;
  }
    ...
    .backup({
      ...
      onDownloadProgress: (completed, totalDownload) => {
        if (listingDone) {
          ...
          if (options.verbose && options.getThrottleSummary) {
            const summary = options.getThrottleSummary();
            if (summary && completed % 10 === 0) {
              // Progress bar uses \r — print stats on their own line occasionally
              process.stderr.write(`\n${chalk.dim(`[throttle] ${summary}`)}\n`);
            }
          }
        }
      },
...
export async function backupCommand(
  ...
  let mode: 'cdp' | 'token' = 'token';

  try {
    // CDP uses a tab pool ≈ --concurrency (capped); --delay is the min start gap
    const concurrency = Math.min(Math.max(requestedConcurrency, 1), 6);
    const delay = Math.max(0, requestedDelay);

    const created = await createChatGptClient({
      token,
      verbose,
      cdpPort,
      cdpPoolSize: concurrency,
      cdpMinGapMs: Math.max(delay, 50),
    });
    client = created.client;
    mode = created.mode;
    const effectiveConcurrency =
      mode === 'cdp' ? concurrency : requestedConcurrency;
    const effectiveDelay = delay;

    spinner.text =
      mode === 'cdp'
        ? `Authenticating via Chromium CDP (${effectiveConcurrency} tabs)...`
        : 'Authenticating via access token...';
    await client.initialize();
    spinner.succeed(
      mode === 'cdp'
        ? `Authenticated (Chromium CDP, ${client.cdpWorkers || effectiveConcurrency} workers)`
        : 'Authenticated (access token)'
    );

    const activeClient = client;
    const getThrottleSummary = () => activeClient.throttleStats?.summary();

    console.log(chalk.dim(`\nBackup settings:`));
    console.log(chalk.dim(`  Output: ${output}`));
    console.log(chalk.dim(`  Concurrency: ${effectiveConcurrency}`));
    console.log(chalk.dim(`  Delay/gap: ${effectiveDelay}ms`));
    if (mode === 'cdp') {
      console.log(
        chalk.dim(
    ...
    if (project) {
      ...
      if (shouldDownloadFiles) {
        await runFileDownloads(client, output, {
          concurrency: effectiveConcurrency,
          delay: effectiveDelay,
          verbose,
        });
    ...
    if (error instanceof Error) {
      if (error.name === 'AuthenticationError') {
        ...
        console.error('  prompt-exporter sync');
        console.error(
          chalk.dim(
            '\nNode Bearer requests are blocked by Cloudflare (HTTP 403).'
          )
        );
      } else {
        console.error(chalk.red(`\nError: ${error.message}`));
