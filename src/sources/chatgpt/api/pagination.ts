...
export async function fetchAllProjects(
  ...
  do {
    ...
    const response: ProjectsSidebarResponse = await client.fetch<ProjectsSidebarResponse>(url, {
...
export async function* fetchProjectConversations(
  ...
  while (cursor !== null && cursor !== undefined) {
    const url: string = `${ENDPOINTS.PROJECT_CONVERSATIONS(gizmoId)}?cursor=${encodeURIComponent(cursor)}`;

    const response: ProjectConversationsResponse = await client.fetch<ProjectConversationsResponse>(url, {
