# prompt-exporter release automation
#
# Order (intentional):
#   1) npmjs.com  — validate/publish the package from the release commit
#   2) GitHub     — annotated tag + Release (source tarball consumers use)
#   3) AUR        — stable package pins the GitHub tag tarball sha256
#
# Usage:
#   make help
#   make check
#   make release          # full pipeline (asks confirmation)
#   make npm-publish
#   make github-release
#   make aur-publish

SHELL := /bin/bash
.SHELLFLAGS := -eu -o pipefail -c

VERSION := $(shell node -p "require('./package.json').version")
TAG     := v$(VERSION)
REPO    := azbarcea/prompt-exporter
REMOTE  := origin
BRANCH  := $(shell git rev-parse --abbrev-ref HEAD)
TARBALL_URL := https://github.com/$(REPO)/archive/refs/tags/$(TAG).tar.gz
AUR_STABLE  := aur/prompt-exporter
AUR_GIT     := aur/prompt-exporter-git

.PHONY: help version check npm-pack npm-publish \
	github-tag github-release \
	aur-sha aur-commit aur-publish \
	release smoke

help:
	@echo "prompt-exporter release (version $(VERSION) → $(TAG))"
	@echo ""
	@echo "Pipeline:  npmjs → GitHub tag/release → AUR (GitHub tarball)"
	@echo ""
	@echo "  make check            typecheck + unit tests; refuse dirty tree"
	@echo "  make npm-pack         dry-run npm pack"
	@echo "  make npm-publish      publish to npmjs.com (public)"
	@echo "  make github-tag       create/push annotated tag $(TAG)"
	@echo "  make github-release   gh release from $(TAG) (CHANGELOG section)"
	@echo "  make aur-sha          set PKGBUILD pkgver + sha256 from GitHub tarball"
	@echo "  make aur-commit       commit AUR metadata (if changed) and push"
	@echo "  make aur-publish      push PKGBUILDs to aur.archlinux.org"
	@echo "  make release          check → npm → github → aur (CONFIRM=1)"
	@echo "  make smoke            print install smoke commands"
	@echo ""
	@echo "Example:  make release CONFIRM=1"

version:
	@echo "$(VERSION)"

check:
	@echo "==> check working tree"
	@test -z "$$(git status --porcelain)" || { \
	  echo "error: working tree not clean; commit or stash first" >&2; \
	  git status -sb; exit 1; \
	}
	@echo "==> on branch $(BRANCH)"
	@npm run typecheck
	@npm test
	@echo "==> package.json version $(VERSION); ensure CHANGELOG has ## $(VERSION)"
	@grep -q "^## $(VERSION)$$" CHANGELOG.md || { \
	  echo "error: CHANGELOG.md missing '## $(VERSION)'" >&2; exit 1; \
	}

npm-pack: check
	@echo "==> npm pack --dry-run"
	@npm pack --dry-run

npm-publish: check
	@echo "==> npm whoami"
	@npm whoami
	@echo "==> npm publish --access public ($(VERSION))"
	@npm publish --access public
	@npm view prompt-exporter version

github-tag: check
	@echo "==> ensure remote $(REMOTE) has latest $(BRANCH)"
	@git push $(REMOTE) $(BRANCH)
	@if git rev-parse "$(TAG)" >/dev/null 2>&1; then \
	  echo "tag $(TAG) already exists locally"; \
	else \
	  git tag -a "$(TAG)" -m "prompt-exporter $(VERSION)"; \
	  echo "created tag $(TAG)"; \
	fi
	@git push $(REMOTE) "$(TAG)"

github-release: github-tag
	@echo "==> GitHub Release $(TAG)"
	@if gh release view "$(TAG)" >/dev/null 2>&1; then \
	  echo "release $(TAG) already exists"; \
	else \
	  NOTES=$$(awk '/^## $(VERSION)$$/{p=1;next} /^## /{if(p) exit} p' CHANGELOG.md); \
	  gh release create "$(TAG)" \
	    --title "$(VERSION)" \
	    --notes "$$NOTES"; \
	fi
	@gh release view "$(TAG)" --web >/dev/null 2>&1 || true
	@echo "https://github.com/$(REPO)/releases/tag/$(TAG)"

aur-sha:
	@echo "==> wait for GitHub tarball $(TARBALL_URL)"
	@for i in 1 2 3 4 5 6; do \
	  CODE=$$(curl -sL -o /tmp/pe-$(TAG).tgz -w '%{http_code}' "$(TARBALL_URL)"); \
	  BYTES=$$(wc -c </tmp/pe-$(TAG).tgz); \
	  if [[ "$$CODE" == "200" && "$$BYTES" -gt 1000 ]]; then break; fi; \
	  echo "  attempt $$i: HTTP $$CODE bytes $$BYTES — retry"; \
	  sleep 2; \
	done
	@SUM=$$(sha256sum /tmp/pe-$(TAG).tgz | awk '{print $$1}'); \
	  echo "sha256=$$SUM"; \
	  sed -i "s/^pkgver=.*/pkgver=$(VERSION)/" $(AUR_STABLE)/PKGBUILD; \
	  sed -i "s/^pkgrel=.*/pkgrel=1/" $(AUR_STABLE)/PKGBUILD; \
	  sed -i "s/sha256sums=.*/sha256sums=('$$SUM')/" $(AUR_STABLE)/PKGBUILD; \
	  ( cd $(AUR_STABLE) && makepkg --printsrcinfo > .SRCINFO ); \
	  _r=$$(git rev-list --count HEAD); \
	  _g=$$(git rev-parse --short HEAD); \
	  sed -i "s/^pkgver=.*/pkgver=$(VERSION).r$${_r}.g$${_g}/" $(AUR_GIT)/PKGBUILD; \
	  ( cd $(AUR_GIT) && makepkg --printsrcinfo > .SRCINFO ); \
	  echo "updated $(AUR_STABLE) and $(AUR_GIT)"

aur-commit:
	@git add $(AUR_STABLE) $(AUR_GIT)
	@if git diff --cached --quiet; then \
	  echo "no AUR metadata changes to commit"; \
	else \
	  git commit -m "chore(aur): set sha256 for prompt-exporter $(VERSION)"; \
	  git push $(REMOTE) $(BRANCH); \
	fi

aur-publish:
	@echo "==> AUR SSH check"
	@ssh -o BatchMode=yes -T aur@aur.archlinux.org 2>&1 | head -3 || true
	@./aur/publish.sh prompt-exporter
	@./aur/publish.sh prompt-exporter-git

# Full ship: npm → GitHub → AUR (from GitHub tag tarball)
release:
	@if [[ "$(CONFIRM)" != "1" ]]; then \
	  echo "Refusing to run full release without CONFIRM=1"; \
	  echo "  make release CONFIRM=1"; \
	  exit 1; \
	fi
	$(MAKE) check
	$(MAKE) npm-publish
	$(MAKE) github-release
	$(MAKE) aur-sha
	$(MAKE) aur-commit
	$(MAKE) aur-publish
	@echo ""
	@echo "Released prompt-exporter $(VERSION)"
	@echo "  npm:    https://www.npmjs.com/package/prompt-exporter"
	@echo "  GitHub: https://github.com/$(REPO)/releases/tag/$(TAG)"
	@echo "  AUR:    https://aur.archlinux.org/packages/prompt-exporter"

smoke:
	@echo "npm:  npm install -g prompt-exporter@$(VERSION) && prompt-exporter --version"
	@echo "AUR:  yay -S prompt-exporter && prompt-exporter --version"
	@echo "src:  prompt-exporter sources"
